var
server_port,password,timeout,method,protocol,obfs,redirect,dns_ipv6,fast_open,workers:string;
begin
assign(output,'/etc/shadowsocks.json');
rewrite(output);      //1、防止反复在文件尾写入；2、防止环境变量更改后配置文件不改变；
//以下均不能读入空串！（输入一个空串=未输入）
readln(server_port);  //e.g. 8388,与容器启动参数中的-p <CONTAINER_PORT> 或 -p <HOT_PORT>:<CONTAINER_PORT>一致
readln(password);     //e.g. mypassword
readln(timeout);      //e.g. 120
...
readln(fast_open);    //e.g. true or false
readln(workers);      //e.g. 1
writeln('{');
writeln('    "server": "0.0.0.0",');
writeln('    "server_ipv6": "::",');
writeln('    "server_port": ',server_port,',');
...
writeln('    "fast_open": ',fast_open,',');
writeln('    "workers": ',workers,'');
write('}');
end.
